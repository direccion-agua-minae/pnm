...
```{r}
valueBox(value = paste(format(archivo_datos_PNM[85,3])),
...
```{r}
valueBox(value = paste(format(archivo_datos_PNM[85,2])),
...
### Cuenca {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[85,1])), 
         caption = "Cuenca",
         color = color_info
)
```
...
```{r}
valueBox(value = paste(format(archivo_datos_PNM[85,12])), 
...
### Coordenadas {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[85,13])), 
...
### Provincia {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[85,15])), 
         caption = "Provincia",
         color = color_ubic
)
```
...
### Cantón {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[85,16])), 
         caption = "Cantón",
         color = color_ubic
)
```
...
### Distrito {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[85,17])), 
         caption = "Distrito",
         color = color_ubic
)
```
...
```{r}
knitr::kable(
  archivo_datos_PNM[85:88, 18:24], bootstrap_options = "basic", 
...
leaflet(data = PNM[118:118,]) %>%
  addTiles() %>%
  addProviderTiles(providers$Stamen.TonerLite, group = "Stamen Toner Lite") %>%
  addProviderTiles(providers$Esri.WorldImagery, group = "Imágenes de ESRI") %>% 
  addMarkers(~LONG, ~LAT, 
             popup= paste(
               "Sitio de muestreo: ", PNM$Codigo[118]
  )) %>%  
  addLayersControl(
