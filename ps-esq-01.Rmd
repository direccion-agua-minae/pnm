...
icono_info1 <- 'fas fa-cogs'
icono_info2 <- 'fas fa-edit'
...
archivo_datos_PNM <- read.csv('https://raw.githubusercontent.com/EsmeVar/PNM_FASE_5/main/2021_04_28_PNM_INFO_F5.csv', sep = ";")
...
```{r}
valueBox(value = paste(format(archivo_datos_PNM[5,3])),
...
```{r}
valueBox(value = paste(format(archivo_datos_PNM[5,2])),
...
### Cuenca {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[5,1])), 
         caption = "Cuenca",
         color = color_info
)
```
...
```{r}
valueBox(value = paste(format(archivo_datos_PNM[5,12])), 
...
### Coordenadas {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[5,13])), 
...
### Provincia {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[5,15])), 
         caption = "Provincia",
         color = color_ubic
)
```
...
### Cantón {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[5,16])), 
         caption = "Cantón",
         color = color_ubic
)
```
...
### Distrito {.value-box}
```{r}
valueBox(value = paste(format(archivo_datos_PNM[5,17])), 
         caption = "Distrito",
         color = color_ubic
)
```
...
```{r}
knitr::kable(
  archivo_datos_PNM[5:8, 18:24 ], caption = '**Utilizando el Índice Holandes y el Índice Biológico (BMWP-CR), según el Decreto Ejecutivo N°33903-MINAE-S**'
)
```
...
leaflet(data = PNM[42:42,]) %>%
  addTiles() %>%
  addProviderTiles(providers$Stamen.TonerLite, group = "Stamen Toner Lite") %>%
  addProviderTiles(providers$Esri.WorldImagery, group = "Imágenes de ESRI") %>% 
  addMarkers(~LONG, ~LAT, 
             popup= paste(
               "Sitio de muestreo: ", PNM$Codigo[42]
  )) %>%  
  addLayersControl(
